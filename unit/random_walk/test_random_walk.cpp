...
real1d compute_cost(realConst2d params, realConst1d params_init) {
  ...
  parallel_for( YAKL_AUTO_LABEL() , params.extent(1) , YAKL_LAMBDA (int iens) {
...
int main( int argc , char **argv ) {
  ...
  yakl::init();
  {
    ...
    portopt::Random_Walk<real> trainer( params_init.createDeviceCopy() , 1e4 , 1.e-2 );

    yakl::timer_start("random");
    for (int igen = 0; igen < num_iterations; igen++) {
    ...
    yakl::timer_stop("random");

    auto ensemble = trainer.get_ensemble();
    auto best_parameters = trainer.get_parameters();
