...
int main( int argc , char **argv ) {
...

  {
    ...
    typedef double real;
    int constexpr Mem = yakl::memHost;
    typedef yakl::Array<real      ,1,Mem> real1d;
    typedef yakl::Array<real const,2,Mem> realConst2d;

    real constexpr constr_tol = 1.e-7;
    ...
    int num_epochs = 1e4;
    portopt::Gradient_Descent_Adam_FD<real,Mem> trainer( parameters_init.createHostCopy() );

    yakl::timer_start("training_time");
    for (int pwr = -3 ; pwr >= -9; pwr--) {
      trainer = portopt::Gradient_Descent_Adam_FD<real,Mem>( trainer.get_parameters() , std::pow(10.,(double)pwr) );
...

  {
    ...
    lbounds = -0.5;
    ubounds = 2;
    portopt::Genetic_Algorithm<real,Mem> trainer( lbounds , ubounds , 1e3 , 5 , 0.1 , 0.1 );
    ...
    auto ensemble = trainer.get_ensemble();
    auto constr = constraint_cost( trainer.get_best_parameters().reshape(num_parameters,1) );
    auto close  = closeness_cost ( trainer.get_best_parameters().reshape(num_parameters,1) );
    std::cout << "Max Constraint Violation: " << yakl::intrinsics::maxval( constr ) << std::endl;
    std::cout << "Closeness Rel L1: "         << yakl::intrinsics::sum(close)/close.size() << std::endl;
  }
